...
| 11 | The breach | Char's call names a hardline access point on the parlor wall. V jacks in and plays the game's own code grid; the breach is the objective. The dump goes to Char, and her text half an hour later names the man | `cc_g02_ap_breached`, `cc_g02_char_named` |
