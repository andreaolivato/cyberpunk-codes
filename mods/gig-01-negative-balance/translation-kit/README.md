# Translating Negative Balance

This guide shows step-by-step instructions on how to translate Negative Balance into
one of the officially supported languages of CP2077.

## What is already translated

For known characters (V, Johnny, Mama Welles and Nix), we are using the game's original
...
- Nix's messages and V's replies
- the objectives and map captions
- the shard
- the terminal documents
- the notices and alerts on screen
- the subtitles for the NPCs invented for this gig: Elena and Hoshino.
- (optional) the voice of Elena and Hoshino.
...
2. In the project explorer, under `archive`, open
   `mod\negative_balance_translation\strings.json`. It lists every string. Open an
...
3. Open `lines.json` in the same folder: the subtitles of Elena and Hoshino:

   ```
   <kiroshi l="eng" o="V? Sorry, I didn't know you'd answer. My name's Elena Ortega." t="V? Sorry, I didn't know you'd answer. My name's Elena Ortega." b="" a=""/>
   ```
   ...
   plain text. Save.
4. Under `resources`, open `0_negative_balance_translation.archive.xl` and replace
...
## Recording Elena and Hoshino

If you are a voice actor, Elena and Hoshino can also be voiced in your
...
`source\archive\mod\negative_balance_translation\vo\` with the names defined in
`vomap.json` (e.g. `gig01_elena_call__e01.wem`). Then copy
