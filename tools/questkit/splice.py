r"""Cutting and joining vanilla voice clips: the shared half of splice_takes.

A character the base game voices speaks only in the game's own recordings.
Most lines are a whole take, and the rest are one take cut at a pause so the
wanted words come free. This module is the cutting; each gig's
`splice_takes.py` is that gig's list of cuts and nothing else.

MOVED HERE 2026-09-06. It was gig 01's `splice_takes.py`, and gig 02 reached
it by loading that file off disk with importlib, which is the shape
`docs/conventions.md` says questkit exists to prevent.
