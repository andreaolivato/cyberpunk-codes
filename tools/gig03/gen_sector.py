r"""Generates the world files that put the gig's shard on top of the cabinet.

  mod\worlds\03_night_city\_compiled\default\cc_g03_world.streamingsector
  mod\worlds\03_night_city\_compiled\default\cc_g03_world.streamingblock

WHAT THIS SHIPS: a `worldEntityNode` carrying the game's own shard case
container, holding the gig's own item, standing on the cabinet in the security
room of the Militech compound. It renders, it offers "F Take / R Read", and
reading it opens the gig's note.

THIS GIG DELETES NOTHING. Gig 01 replaced a shard the base game had already put
on that desk, so its manifest removes the original. Ours stands somewhere the
base game put nothing, so there is no `streaming.sectors` deletion block and no
instance count to keep in step with a patch.

AN EARLIER PLAN PUT THE SHARD INSIDE THE CABINET rather than on it, by pushing
the item into the container's inventory. It was dropped on 2026-09-09 for a
reason worth keeping: a container the player has already looted may never open
again, so the shard would be there on a clean save and missing on a save where
the room had been cleared out first. Placing our own object has none of that,
and it is the route this playbook already proves.
...
RAW = os.path.join(REPO, 'mods', 'gig-03-acceptable-loss', 'source', 'wkit',
                   'raw', 'mod', 'worlds', '03_night_city', '_compiled', 'default')

SECTOR_DEPOT = r'mod\worlds\03_night_city\_compiled\default\cc_g03_world.streamingsector'
...
# THE LIFTED VANILLA CONTAINER IS SHARED WITH GIG 01, DELIBERATELY. It is the
# game's own node with all 53 fields of its instance data, and nothing about it
# belongs to a gig: only `itemTDBID`, the position and the name are ours. A
# second copy would be a second thing to keep in step with the vanilla sector it
# came from, and re-extracting it needs the game and WolvenKit for no gain.
#
# `--refresh` still lives in gig 01, which is where the extraction is written
# down. If this file cannot find the JSON, run that one.
SOURCE = os.path.join(_TOOLS, 'gig01', 'shard_container_node.json')
...
SHARD_REF = '$/03_night_city/se5/#cc_g03_shard_container'
...
# THE HEIGHT IS THE ONE NUMBER HERE THAT WAS NOT CAPTURED. x and y are the
# cabinet's own, read off the device probe. The z started as the cabinet's
# origin plus 0.90 m, which floated it a hand's width above the surface in
# playtest, and is 0.75 m now.
SHARD_POS = (-95.621, -5211.431, 87.885)
...
def vanilla_container():
    ...
    # THIS GIG DELETES NOTHING, so those two numbers are not wanted in the
    # manifest. They are printed because gig 01 needs them, and because a lift
    # that silently changed which node it took would show up here.
    print('  (gig 01 needs expectedNodes %d / index %d for its deletion; '
          'this gig has no deletion block)'
...
def sector():
    ...
    node['debugName'] = cname('{cc_g03_shard}')
    ...
    return {
        'Header': header('cc_g03_world.streamingsector'),
        'Data': {
...
def block():
    return {
        'Header': header('cc_g03_world.streamingblock'),
        'Data': {
            'Version': 195, 'BuildVersion': 0,
            'RootChunk': {
...
if __name__ == '__main__':
    ...
    for name, doc in (('cc_g03_world.streamingsector.json', sector()),
                      ('cc_g03_world.streamingblock.json', block())):
    ...
    print('holding', SHARD_ITEM, '- read with [F]/[R], see Gig03_Shard.reds')
